	...
	this.resultHandler = function (LOG_TITLE,callback, errorHandler) {
		return function (err, res, result) {
			if (!err && res.statusCode === 200) {
				...
				if (typeof errorHandler === 'function'){errorHandler(errorMsg) ;}
				else{
					console.error(LOG_TITLE,'failed with error'+errorMsg);
				}
...
RESTClient.prototype.create = function (data, callback, errorHandler) {
 ...
 RESTClient.prototype.byName = function(name,callback,errorHandler){
	...
	if(this.extraInfo){
		var path = this.path + '/' + encodeURIComponent(name) + '?key=' + this.key+'&tenantId='+this.tenantId+this.extraInfo;
	}else{
...
function DeedManager(gamiRESTConf, 
	...
	this.turnOn = function(deedName,uid,callback,errorHandler){
		self.path = '/trigger/plan/'+self.planName+'/deed/'+deedName;
		var data = {uid:uid};
...
function EventManager(gamiRESTConf, 
	...
	this.fireEvent2 = function(eventName,eventSource,uid,param,callback,errorHandler){
		self.path =  '/trigger/plan/'+self.planName+'/event/'+eventName;
		var data = {uid:uid,eventSource:eventSource,params:param};
...
function UserManager(gamiRESTConf, 
	...
	this.extraInfo = "&extraInfo=ONGOING_MISSION";
